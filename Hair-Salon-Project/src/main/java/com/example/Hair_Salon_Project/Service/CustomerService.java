...
public class CustomerService  {
    ...
    public Booking createNewBooking(BookingRequest bookingRequest){
        if (bookingRequest.getBookingDate() == null || bookingRequest.getProductType() == null || bookingRequest.getProduct() == null ) {
            ...
            try {
                ...
                Product product = productRepository.findProductByTypeAndId(bookingRequest.getProduct().getType(),bookingRequest.getProduct().getId());
                if (product == null || !product.getType().equals(bookingRequest.getProductType())) {
                ...
                if (bookingRequest.getStaff() != null) {
                    Staff selectedStaff = staffRepository.findById(bookingRequest.getStaff().getId())
...
//                        throw new IllegalArgumentException("Người dùng đã chọn không phải là stylist.");
//                    }
    ...
    public List<BookingStatus> getAllBooking() {
        try {
            Account accountRequest = authenticationService.getCurrentAccount();
            List<Booking> bookingList = bookingRepository.findBookingByAccount_Id(accountRequest.getId());
            ...
            for(Booking booking: bookingList){
                ...
                bookingStatus.setStaffName(booking.getStaff().getFirstName() + " " + booking.getStaff().getLastName());
