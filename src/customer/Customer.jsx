...
const Customer = () => {
    const { customers, handleDeleteCustomer, loading } = useContext(CustomerContext);
    ...
    const allCustomersSelected = customers.length > 0 && selectedCustomers.length === customers.length;
                    ...
                    <div className="d-flex align-items-center my-4" style={{ placeContent: 'end' }}>
                        ...
                        <Dropdown>
                            <Dropdown.Toggle variant="outline-dark" id="dropdown-basic" style={{ width: '170px' }}>
                                {sortOption === 'no-asc' ? 'No (Ascending)' :
                                    sortOption === 'no-desc' ? 'No (Descending)' :
                                                    ...
                                                    sortOption === 'email-desc' ? 'Email (Z-A)' :
                                                        sortOption === 'phone-asc' ? 'Phone (A-Z)' :
                                                            sortOption === 'phone-desc' ? 'Phone (Z-A)' :
                                                                sortOption === 'active-asc' ? 'Active (Yes first)' :
                                                                    sortOption === 'active-desc' ? 'Active (No first)' :
                                                                        sortOption === 'point-asc' ? 'Point (Low to High)' :
                                                                            sortOption === 'point-desc' ? 'Point (High to Low)' : 'Sort By'}
                            </Dropdown.Toggle>

                            <Dropdown.Menu>
                                <Dropdown.Item
                                    className={`custom-dropdown-item ${sortOption === 'no-asc' ? 'custom-active' : ''}`}
                                    onClick={() => changeSortOption('no-asc')}
                                >
                                    No (Ascending)
                                </Dropdown.Item>
                                <Dropdown.Item
                                    className={`custom-dropdown-item ${sortOption === 'no-desc' ? 'custom-active' : ''}`}
                                ...
                                <Dropdown.Item
                                    className={`custom-dropdown-item ${sortOption === 'email-asc' ? 'custom-active' : ''}`}
                                    onClick={() => changeSortOption('email-asc')}
                                >
                                    Email (A-Z)
                                </Dropdown.Item>
                                <Dropdown.Item
                                    className={`custom-dropdown-item ${sortOption === 'email-desc' ? 'custom-active' : ''}`}
                                    onClick={() => changeSortOption('email-desc')}
                                >
                                    Email (Z-A)
                                </Dropdown.Item>
                                <Dropdown.Item
                                    className={`custom-dropdown-item ${sortOption === 'phone-asc' ? 'custom-active' : ''}`}
                                    onClick={() => changeSortOption('phone-asc')}
                                >
                                    Phone (A-Z)
                                </Dropdown.Item>
                                <Dropdown.Item
                                    className={`custom-dropdown-item ${sortOption === 'phone-desc' ? 'custom-active' : ''}`}
                                    onClick={() => changeSortOption('phone-desc')}
                                >
                                    Phone (Z-A)
                                </Dropdown.Item>
                                <Dropdown.Item
                                    className={`custom-dropdown-item ${sortOption === 'active-asc' ? 'custom-active' : ''}`}
                                    onClick={() => changeSortOption('active-asc')}
                                >
                                    Active (Yes first)
                                </Dropdown.Item>
                                <Dropdown.Item
                                    className={`custom-dropdown-item ${sortOption === 'active-desc' ? 'custom-active' : ''}`}
                                    onClick={() => changeSortOption('active-desc')}
                                >
                                    Active (No first)
                                </Dropdown.Item>
                                <Dropdown.Item
                                    className={`custom-dropdown-item ${sortOption === 'point-asc' ? 'custom-active' : ''}`}
                                    onClick={() => changeSortOption('point-asc')}
                                >
                                    Point (Low to High)
                                </Dropdown.Item>
                                <Dropdown.Item
                                    className={`custom-dropdown-item ${sortOption === 'point-desc' ? 'custom-active' : ''}`}
                        ...
                        <tbody>
                            {sortedCustomers.map((cust) => (
                                <tr key={cust.id}>
                                    ...
                                    <td style={{ alignContent: 'center', height: '100px' }}>{cust.id}</td>
                                    <td style={{ alignContent: 'center', height: '100px' }}>{cust.name}</td>
                                    <td style={{ alignContent: 'center', height: '100px' }}>{cust.email}</td>
                                    <td style={{ alignContent: 'center', height: '100px' }}>{cust.phone}</td>
                                    <td style={{ alignContent: 'center', height: '100px' }}>
                                        {cust.is_active === true ?
                                            <div style={{ backgroundColor: 'green', color: 'white', borderRadius: '1rem', margin: '30px' }}>
                                                Active
                                            </div> : <div style={{ backgroundColor: '#cf2626', color: 'white', borderRadius: '1rem', margin: '30px' }}>
                                                No Active
                                            </div>}
                                    </td>
                                    <td style={{ alignContent: 'center', height: '100px' }}>{cust.point}</td>
                                    <td style={{ alignContent: 'center', height: '100px' }}>
                                        <Dropdown>
                                            <Dropdown.Toggle
                                                variant="link"
                                            ...
                                            <Dropdown.Menu>
                                                <Dropdown.Item onClick={() => handleShowModal(cust)}>View Information</Dropdown.Item>
                ...
                <Modal.Body>
                    {currentRequest && (
                        <>
                            ...
                            <p><strong>Sex:</strong> {currentRequest.sex}</p>
                            <p><strong>Birth Date:</strong> {currentRequest.birth_date}</p>
                            <p><strong>Email:</strong> {currentRequest.email}</p>
                            <p><strong>Phone:</strong> {currentRequest.phone}</p>
                            <p><strong>Point:</strong> {currentRequest.point}</p>
                            <p><strong>Status:</strong> {currentRequest.is_active ?
                                <span style={{ backgroundColor: 'green', color: 'white', padding: '2px 30px', borderRadius: '1rem' }}>Active</span>
                                :
                                <span style={{ backgroundColor: '#cf2626', color: 'white', padding: '2px 20px', borderRadius: '1rem' }}>No Active</span>}
                            </p>
                        </>
                    )}
